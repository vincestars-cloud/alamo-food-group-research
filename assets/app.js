...
function ownerOrders(el){
  ...
  ${orders.map(o=>`<tr><td><strong>${o.id}</strong></td><td>${o.date}</td><td>${o.customer}</td><td>${o.items.length}</td><td>$${o.total.toFixed(2)}</td><td><span class="d-status st-${o.status==='delivered'?'delivered':o.status==='transit'?'transit':'processing'}">${o.status}</span></td><td>${o.status!=='delivered'?`<select onchange="updateOrderStatus('${o.id}',this.value)" style="font-size:0.75rem;padding:4px 8px;border:1px solid var(--gray-300);border-radius:4px"><option value="processing" ${o.status==='processing'?'selected':''}>${T('Processing','Procesando')}</option><option value="transit" ${o.status==='transit'?'selected':''}>${T('In Transit','En Tránsito')}</option><option value="delivered" ${o.status==='delivered'?'selected':''}>${T('Delivered','Entregado')}</option></select>`:''}</td></tr>`).join('')}
  </tbody></table></div>`;
}
...
function custOverview(el){
  ...
  <div class="dash-grid"><div><div class="dash-panel"><h4>${T('Recent Orders','Pedidos Recientes')}</h4>${myOrders.slice(0,5).map(o=>`<div class="d-row"><div><strong>#${o.id}</strong><br><span style="font-size:0.75rem;color:var(--gray-400)">${o.date} — ${o.items.length} ${T('items','artículos')}</span></div><div style="text-align:right">$${o.total.toFixed(2)}<br><span class="d-status st-${o.status==='delivered'?'delivered':o.status==='transit'?'transit':'processing'}">${o.status}</span></div></div>`).join('')||`<p style="color:var(--gray-400)">${T('No orders yet','Sin pedidos aún')}</p>`}<div style="margin-top:12px"><button class="btn btn-primary btn-sm" onclick="setDashTab('shop')">${T('Place New Order','Hacer Nuevo Pedido')}</button></div></div></div>
...
function custOrders(el){
  ...
  ${my.map(o=>`<tr><td><strong>${o.id}</strong></td><td>${o.date}</td><td>${o.items.length}</td><td>$${o.total.toFixed(2)}</td><td><span class="d-status st-${o.status==='delivered'?'delivered':o.status==='transit'?'transit':'processing'}">${o.status}</span></td><td style="font-size:0.75rem;color:var(--gray-400)">${o.items.map(i=>`${i.qty}x ${i.name}`).join(', ')}</td></tr>`).join('')||`<tr><td colspan="6" style="text-align:center;color:var(--gray-400)">${T('No orders yet','Sin pedidos aún')}</td></tr>`}
  </tbody></table></div>
